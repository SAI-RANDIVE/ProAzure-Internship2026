...
const features = [
  ...
  { icon: <Shield className="w-5 h-5" />, title: 'Secure Access', desc: 'Sign in with your email and password. Your instructor data is completely isolated and safe.' },
      ...
      <section id="how" className="py-20 border-t border-white/5" style={{ background: 'rgba(255,255,255,0.02)' }}>
        <div className="max-w-6xl mx-auto px-6">
          <motion.div
            ...
            className="text-center mb-14"
          >
            <p className="text-sm font-medium text-[#2979ff] mb-3 tracking-widest uppercase">4 steps</p>
            <h2 className="text-3xl md:text-4xl font-semibold">From raw data to full analytics</h2>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
            {[
              { n: '01', title: 'Instructor Sign Up', desc: 'Register using your full name and email address. This creates your dedicated, secure workspace.' },
              { n: '02', title: 'Create a Batch', desc: 'Set up a new class. Give it a name, select the mode (Online, Offline, or Hybrid), and define the schedule.' },
              { n: '03', title: 'Upload Attendance', desc: 'Export the raw attendance CSV directly from Zoom or Google Meet. Navigate to "Upload CSV" and select your batch and platform.' },
              { n: '04', title: 'What to Avoid', desc: 'Do NOT open or edit the CSV files before uploading. Ensure you select the correct source (Zoom vs Meet) to prevent data corruption.' },
            ].map((s, i) => (
              <motion.div
                key={s.n}
                custom={i * 0.1}
                ...
                whileInView="show"
                viewport={{ once: true }}
                className="flex flex-col p-6 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-colors"
              >
                <div className="text-4xl font-bold gradient-text mb-4 font-display text-[#1de9b6]">{s.n}</div>
